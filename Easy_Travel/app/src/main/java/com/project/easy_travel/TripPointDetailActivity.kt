...
class TripPointDetailActivity : AppCompatActivity() , OnMapReadyCallback {
    ...
    private lateinit var point_ : Point
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        titleList.add(point_.name)
        descriptionList.add(point_.describe)
        dateListStart.add(point_.startDate)
        dateListFinish.add(point_.finishDate)
        dateListStartString.add("Data rozpoczęcia: "+sdf.format(point_.startDate).toString())
        dateListFinishString.add("Data zakonczenia: "+sdf.format(point_.finishDate).toString())

        if (point_.lat != 0.0 && point_.lng != 0.0) {
            if (point_.startDate <= currentDate.time && point_.finishDate >= currentDate.time)
                googleMap.addMarker(
                    MarkerOptions()
                        .position(point_.toLatLng())
                        .title(point_.name)
                        .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN))
                )
            else if (point_.startDate > currentDate.time)
                googleMap.addMarker(
                    MarkerOptions()
                        .position(point_.toLatLng())
                        .title(point_.name)
            ...
            else if (point_.finishDate <= currentDate.time)
                googleMap.addMarker(
                    MarkerOptions()
                        .position(point_.toLatLng())
                        .title(point_.name)
            ...
            else if (point_.startDate == currentDate.time || point_.finishDate == currentDate.time
                || (point_.startDate < currentDate.time && point_.finishDate > currentDate.time)
            )
                googleMap.addMarker(
                    MarkerOptions()
                        .position(point_.toLatLng())
            ...
            builder.include(point_.toLatLng())
